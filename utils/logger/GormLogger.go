...
func (l *gormLogger) Print(values ...interface{}) {
	entry := l.logger.WithField("db-name", l.name)
	if len(values) > 1 {
		level := values[0]
		source := values[1]
		entry = l.logger.WithField("db-source", source)
		if level == "sql" {
			duration := values[2]
			// sql
			...
			params := fmt.Sprintf(sqlRegexp.ReplaceAllString(values[3].(string), "%v"), formattedValues...)
			entry.WithField("db-took", duration).Debug(params)
		} else {
