...
func (this *OAuthClient) CreateHMToken(
    ...
    form := url.Values{}
    for s, s2 := range oauthParamsForm {
        form.Add(s, s2)
    }
    req, errr := http.NewRequest("POST", this.GetTokenURL, strings.NewReader(form.Encode()))
    if errr != nil {
        err := fault.ExceptionInternalErrorE(errr)
        return nil, err
    }
    ...
    hc := http.Client{}
    response, errp := hc.Do(req)
    if errp != nil {
        err := fault.ExceptionInternalErrorE(errp)
        return nil, err
    }
    ...
    if response.StatusCode != http.StatusOK {
        ...
        if err != nil {
            err := fault.ExceptionInternalErrorE(err)
            return nil, err
        }

        errResponseErrParse := json.Unmarshal(body, &errResponse)
        if errResponseErrParse != nil {
            rlog.Error(errResponseErrParse)
        }
        if errResponseErrParse == nil {
    ...
    body1, err4 := ioutil.ReadAll(response.Body)
    if err4 != nil {
        err := fault.ExceptionInternalErrorE(err4)
        return nil, err
    }

    erri := json.Unmarshal(body1, &token)
    if erri != nil {
        err := fault.ExceptionInternalErrorE(erri)
        return nil, err
    }
